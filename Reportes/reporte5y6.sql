...
BEGIN
    OPEN ORACLE_REF_CURSOR FOR
        ...
        WHERE (((param_estado IS null) AND (param_fecha is null)) AND (u.cedula = zu.CEDULA_USUARIO AND zu.ID_ESTADO IS NOT NULL AND prov.id = u.ID_PROVEEDOR AND e.ID = zu.ID_ESTADO AND zu.ID_PROVEEDOR_USUARIO = u.ID_PROVEEDOR))
               ...
               OR (((param_estado IS null) AND (TO_DATE(u.FECHA_REGISTRO) = TO_DATE(param_fecha))) AND (u.cedula = zu.CEDULA_USUARIO AND zu.ID_ESTADO IS NOT NULL AND prov.id = u.ID_PROVEEDOR AND e.ID = zu.ID_ESTADO AND zu.ID_PROVEEDOR_USUARIO = u.ID_PROVEEDOR))
