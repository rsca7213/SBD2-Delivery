...
BEGIN
    OPEN ORACLE_REF_CURSOR FOR
        SELECT e.datos_ubicacion.nombre Estado,(SELECT produ.datos_empresa.logo AS logo FROM productores produ WHERE produ.id = prod.id) Empresa,prov.datos_empresa.nombre "Nombre Proveedor de Servicio", (SELECT prove.datos_empresa.logo AS logo FROM proveedores prove WHERE prove.id = prov.id) "Logo Proveedor de Servicio",
        mun.datos_ubicacion.nombre Municipio,
        CASE (SELECT COUNT(*) FROM pedidos pped where pped.id_zona_destino = ped.id_zona_destino and pped.ID_PROVEEDOR_USUARIO = ped.ID_PROVEEDOR_USUARIO and pped.ID_PRODUCTOR_CONTRATO = ped.ID_PRODUCTOR_CONTRATO AND pped.id_municipio_destino = ped.ID_MUNICIPIO_DESTINO)
            WHEN 1 THEN  (SELECT COUNT(*) FROM pedidos pped where pped.id_zona_destino = ped.id_zona_destino and pped.ID_PROVEEDOR_USUARIO = ped.ID_PROVEEDOR_USUARIO and pped.ID_PRODUCTOR_CONTRATO = ped.ID_PRODUCTOR_CONTRATO AND pped.id_municipio_destino = ped.ID_MUNICIPIO_DESTINO) || ' unidad'
            ELSE (SELECT COUNT(*) FROM pedidos pped where pped.id_zona_destino = ped.id_zona_destino and pped.ID_PROVEEDOR_USUARIO = ped.ID_PROVEEDOR_USUARIO and pped.ID_PRODUCTOR_CONTRATO = ped.ID_PRODUCTOR_CONTRATO AND pped.id_municipio_destino = ped.ID_MUNICIPIO_DESTINO) || ' unidades' END "Cantidad de Envios Recibidos"
        ...
        GROUP BY mun.id,prov.datos_empresa.nombre, prod.datos_empresa.nombre, ped.ID_PROVEEDOR_USUARIO, ped.ID_PRODUCTOR_CONTRATO, ped.ID_ZONA_DESTINO, e.datos_ubicacion.nombre, mun.datos_ubicacion.nombre, ped.ID_MUNICIPIO_DESTINO, prod.id, prov.id
        ORDER BY "Cantidad de Envios Recibidos" desc,mun.id, ped.ID_PROVEEDOR_USUARIO, ped.ID_PRODUCTOR_CONTRATO;
END;
...
BEGIN
    OPEN ORACLE_REF_CURSOR FOR
        SELECT u.CEDULA CEDULA, (SELECT us.FOTO AS logo FROM usuarios us WHERE us.CEDULA = u.CEDULA AND rownum < 2) Foto , u.PRIMER_NOMBRE "Primer Nombre" , NVL(u.SEGUNDO_NOMBRE,' ') "Segundo Nombre",
               ...
               prov.DATOS_EMPRESA.NOMBRE "Nombre de la aplicacion registrada", TO_CHAR(u.FECHA_REGISTRO, 'DD/MM/YYYY') "Cliente Desde", e.DATOS_UBICACION.NOMBRE Estado, ( SELECT * FROM (SELECT uz.direccion FROM zonas_usuarios uz WHERE uz.ID_ZONA = zu.ID_ZONA AND uz.ID_PROVEEDOR_USUARIO = zu.ID_PROVEEDOR_USUARIO AND uz.CEDULA_USUARIO = zu.CEDULA_USUARIO ORDER BY DBMS_RANDOM.RANDOM) WHERE ROWNUM < 2) Direccion
        ...
        GROUP BY u.CEDULA, u.PRIMER_NOMBRE, u.SEGUNDO_NOMBRE, u.PRIMER_APELLIDO, u.SEGUNDO_APELLIDO, u.EMAIL, u.ID_PROVEEDOR, prov.DATOS_EMPRESA.NOMBRE,
                 prov.id, e.DATOS_UBICACION.NOMBRE, zu.CEDULA_USUARIO, zu.ID_PROVEEDOR_USUARIO, zu.ID_ZONA, u.FECHA_REGISTRO
