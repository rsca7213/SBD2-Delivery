...
BEGIN
    OPEN ORACLE_REF_CURSOR FOR
        ...
        zTD.DATOS_UBICACION.NOMBRE AS zonaDestino,
            ...
            CASE t.tipo WHEN 'bic' THEN ROUND((ABS(zTO.DATOS_UBICACION.LATITUD - zTD.DATOS_UBICACION.LATITUD) + ABS(zTO.DATOS_UBICACION.LONGITUD - zTD.DATOS_UBICACION.LONGITUD)) * 300 + CEIL(DBMS_RANDOM.VALUE(14,16)))
                    WHEN 'mot' THEN ROUND((ABS(zTO.DATOS_UBICACION.LATITUD - zTD.DATOS_UBICACION.LATITUD) + ABS(zTO.DATOS_UBICACION.LONGITUD - zTD.DATOS_UBICACION.LONGITUD)) * 300 *0.5 + CEIL(DBMS_RANDOM.VALUE(5,10))) ELSE
                        ROUND((ABS(zTO.DATOS_UBICACION.LATITUD - zTD.DATOS_UBICACION.LATITUD) + ABS(zTO.DATOS_UBICACION.LONGITUD - zTD.DATOS_UBICACION.LONGITUD)) * 300 *0.7 + CEIL(DBMS_RANDOM.VALUE(10,13))) END || ':'|| CEIL(DBMS_RANDOM.VALUE(1,58)) || ' min' tiempoEstimado, DECODE(t.tipo,'car','Carro','bic','Bicicleta','mot','Moto','cam','Camioneta') AS tipoTransporte
